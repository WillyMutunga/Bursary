    ...
    public function recordDecision(Request $request, $id, AwardLetterService $awardService)
    {
        try {
            ...
            if ($user) {
                ...
                $fallback = \App\Models\User::whereIn('role', ['admin', 'committee_member'])->first()
                    ?: \App\Models\User::first();
                $userId = $fallback ? $fallback->id : 1;
                $userName = $fallback ? $fallback->name : 'Willy (Committee Member)';
            }
            ...
            if ($request->decision === 'APPROVE') {
                try {
                    ...
                    \Illuminate\Support\Facades\Log::warning("Award letter payload warning: " . $e->getMessage());
                }
                ...
                try {
                    ...
                    \Illuminate\Support\Facades\Log::warning("Notification dispatch warning: " . $e->getMessage());
                }
            } else {
                try {
                    Notification::create([
            ...
            try {
                ...
                \Illuminate\Support\Facades\Log::warning("Audit log warning: " . $e->getMessage());
            }

            return response()->json([
                'success' => true,
                'message' => "Decision recorded successfully. Application {$application->application_no} updated to {$request->decision} (KSh " . number_format($approved) . ").",
                'decision' => $decision,
                'application' => $application->fresh(['cycle', 'ward', 'institution', 'committeeDecisions']),
            ]);
        } catch (\Throwable $e) {
            \Illuminate\Support\Facades\Log::error("Committee decision error: " . $e->getMessage());
            return response()->json([
                'success' => false,
                'message' => 'Unable to record committee decision: ' . $e->getMessage(),
            ], 500);
        }
    }
}
